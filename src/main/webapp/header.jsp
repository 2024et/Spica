<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="jakarta.tags.core" %>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Spica | ヘッダー</title>
<link rel="stylesheet" href="css/header.css">
<link rel="icon" href="images/spica.ico">
</head>
<body>
<header>
<div class="header">
	<div class="information">
		<h4><a href="financialServlet" class="app_name">Spica | 会計管理Webアプリ</a></h4>
		<h4 class="group_name">団体名：${accountData.group_name}</h4>
		<h4 class="user_name">氏名：${accountData.name}</h4>
	</div>
	<div class="navigation">
		<div class="nav_up">
			<a class="empty"></a>
			<a class="empty"></a>
			<a class="empty"></a>
			<c:choose>
			    <c:when test="${accountData.role == '一般'}">
					<a class="empty"></a>
			    </c:when>
			    <c:when test="${accountData.role != '一般'}">
					<a href="managementServlet" class="admin">管理者</a>
			    </c:when>
			</c:choose>
			
			<a href="logoutServlet" class="general">ログアウト</a>
		</div>
		<div class="nav_down">
			<a href="financialServlet" class="general">収支一覧</a>
			<a href="budget_report_listServlet" class="general">予算計画書</a>
			<a href="account_report_listServlet" class="general">会計報告書</a>			
			<a href="purchase_request_listServlet" class="general">購入申請</a>
			<a href="accountServlet" class="general">アカウント</a>
		</div>
	</div>
</div>
</header>
</body>
</html>